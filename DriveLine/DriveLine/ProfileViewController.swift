...
class ProfileViewController: UIViewController, UIScrollViewDelegate {
    ...
    @IBOutlet weak var btnUserFollowers: UIButton!
    ...
    @IBAction func ActionLogout(_ sender: UIBarButtonItem) {
        ...
        self.view.window?.rootViewController = self.storyboard?.instantiateViewController(withIdentifier: "mainNav") as! UINavigationController
    }

    func initViews() {
        userImageView.layer.cornerRadius = 50 / 2
        userImageView.clipsToBounds = true
        let tapgesture = UITapGestureRecognizer.init(target: self, action: #selector(ProfileViewController.editUserImage))
    ...
    override func viewWillAppear(_ animated: Bool) {
        ...
        if model.followers() == "0" || model.followers() == "1" {
            btnUserFollowers.setTitle("\(model.followers()) FOLLOWER", for: .normal)
        }else{
            btnUserFollowers.setTitle("\(model.followers()) FOLLOWERS", for: .normal)
        }
    ...
    func callMyDriveApi() {
        ...
        WebserviceUtil.callGet(httpRequest: URLConstant.API.GET_MYDRIVE, params: ["tag": "showmydrive", "creator_id": uid, "user_id":uid], success: { (response) in
            UIUtil.hideProcessing()
            if let json = response as? NSDictionary {
                if WebserviceUtil.isStatusOk(json: json) {
    ...
    func commentsClick(sender: UIButton) {
        ...
        performSegue(withIdentifier: "PROFILE_COMMENT", sender: self)
    }
    
    
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        if let identifier = segue.identifier {
            if identifier == "PROFILE_COMMENT" {
            ...
            }else if identifier == "PROFILE_DRIVE"{
    ...
    func driveItClick(sender: UIButton) {
        ...
        performSegue(withIdentifier: "PROFILE_DRIVE", sender: self)
    }
    ...
    func deleteClick(sender: UIButton) {
        ...
        let dict = driveArray.object(at: (indexPath?.row)!) as! NSDictionary
        let params = [
            URLConstant.Param.TAG: "deletedrive",
            "creator_id": model.userid(),
            "drive_id": String((dict.object(forKey: "driveid") as! NSString).intValue)
            ] as [String : String]
        UIUtil.showProcessing(message: "Processing...")
        WebserviceUtil.callPost(httpRequest: URLConstant.API.DELETE_DRIVE, params: params, success: { (response) in
            UIUtil.hideProcessing()
            if let json = response as? NSDictionary {
                if WebserviceUtil.isStatusOk(json: json) {
                    self.callMyDriveApi()
...
//        let proview = sender.superview?.viewWithTag(index + 8000) as! UIProgressView
        progressBar = cell.progressview
