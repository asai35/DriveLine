...
class DriveViewController: UIViewController, UITextFieldDelegate, CLLocationManagerDelegate, MKMapViewDelegate{
    ...
    func rightBarButton(_ sender: UIBarButtonItem){
...
//        SharedAppDelegate.stopContinuousLocation(locationManager: self.locationManager, requestId: self.locationRequestId)
    }
...
//
    ...
    func showDialogToCaptureVideo() {
        let alert = UIAlertController(title: "Video Recording", message: "Would you like to use your phones camera to record video of your drive?", preferredStyle: .alert)
...
//                self.imagePicker.videoMaximumDuration = 1 * 60
                self.imagePicker.videoMaximumDuration = 60 * 60 // 7 Mins durations
    ...
    @IBAction func driveStartClick(_ sender: Any) {
        if isTracking {
            ...
            if isEnd {
                ...
                for p in self.coordinatesList {
                    area.append(CLLocationCoordinate2DMake(p.coordinate.latitude, p.coordinate.longitude))
                }
                let polyline = MKPolyline(coordinates: &area, count: area.count)
                let rect = MKMapRectMake(polyline.boundingMapRect.origin.x - polyline.boundingMapRect.size.width * 0.15, polyline.boundingMapRect.origin.y - polyline.boundingMapRect.size.height * 0.15, polyline.boundingMapRect.size.width * 1.3, polyline.boundingMapRect.size.height * 1.3)
            ...
            if (self.coordinatesList.count > 0) {
                self.coordinatesList.removeAll()
            }
            isEnd = false
            self.coordinatesList = [CLLocation]()
