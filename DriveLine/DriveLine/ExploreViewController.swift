...
//  Copyright © 2017 DriveLine. All rights reserved.
//
...
class ExploreViewController: UIViewController {
    ...
    func initViews() {
        ...
        self.styleButton.setTitle("All", for: .normal)
    ...
    @IBAction func stylesClick(_ sender: UIButton) {
        ...
        var array = [String]()
        array.append("All")
        for s in Constant.Data.DRIVE_STYLES {
            array.append(s)
        }
        self.selectedStyle = "All"
        ActionSheetStringPicker.show(withTitle: "Select Drive Style", rows: array, initialSelection: 0, doneBlock: { (action, index, value) in
...
//        points = event?["points"] //"25 bonus points"
        ...
        if event?["isjoined"] == "0" {
            ...
            progressView.progress = CGFloat(Float(event!["progress"]!)!) / 100
            progressViewText.text = "\(Int(Float(event!["progress"]!)!))% Complete"
            ...
            if Float(event!["progress"]!)! > 100 {
                joinButton.isHidden = true
                progressViewText.text = "100% Complete"
            }else{
    ...
    func showProgressView(progress: Float) {
        ...
        self.progressViewText.text = "\(Int(progress))% Complete"
