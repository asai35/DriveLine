...
//  AACameraView
//
//  Created by HeliumSoft. Ahsan Ali on 04/03/2017.
//  Copyright © 2017 HeliumSoft. All rights reserved.
//
...
@IBDesignable open class AACameraView: UIView {
    ...
    var output: AVCaptureOutput = AVCaptureMovieFileOutput() {
        didSet {
    ...
    open var outputMode: OUTPUT_MODE = .videoAudio {
        didSet {
    ...
    open func startVideoRecording() {
        ...
        output.startRecording(toOutputFileURL: tempMoviePath(name: videoName), recordingDelegate: self)
    }
    
    open func setVideoName(name: String) {
        videoName = name
    }
    
    open func getVideoName() -> String {
        return videoName
    }
    
    func tempMoviePath(name:String)-> URL {
        let tempPath = URL(fileURLWithPath: NSTemporaryDirectory()).appendingPathComponent(name).appendingPathExtension("mp4").absoluteString
...
extension AACameraView {
    ...
    func setSession() {
        ...
        global.queue.async(execute: {
            ...
            self.previewLayer = session.setPreviewLayer(self, orientation: self.videoOrientation!)
            self.previewLayer?.frame = self.bounds
            for connections in self.output.connections{
                for port in (connections as! AVCaptureConnection).inputPorts{
                    if (port as AnyObject).mediaType == AVMediaTypeVideo{
                        self.videoConnection = connections as? AVCaptureConnection
                        break;
                    }
                }
            }
