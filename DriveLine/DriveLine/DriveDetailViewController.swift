...
class DriveDetailViewController: UIViewController {
    ...
    func initViews() {
        ...
        if URL(string: myDrive["creatorphoto"] as! String) == nil {
            userImageView.image = #imageLiteral(resourceName: "person-avatar")
        }else{
            userImageView.setImageWith(URL(string: myDrive["creatorphoto"] as! String)!, placeholderImage: UIImage(named: "person-avatar"))
        }
        ...
        driveLength.text = String(format: "%.3f miles", length)
    ...
    func showLocationOnMap(lat: Double, lng: Double) {
        ...
        driveDistance.text = String(Int(distancehere))+" miles to the start"
    }
